...
def news_section():
    ...
    for entry in entries:
        ...
        title = escape(entry.get('prefix', '')) + title + escape(entry.get('suffix', ''))
    ...
    return '<section class="home-news" aria-labelledby="news-title"><h2 id="news-title">News</h2>' + content + '</section>'
...
opened=False
conference_start = next(i for i, item in enumerate(b) if clean(item['text']) == 'Conferences') + 1
seminars_start = next(i for i, item in enumerate(b) if clean(item['text']) == 'Seminars')
for block in b[conference_start:seminars_start]:
...
contact=f'''<div class="wrap contact-content"><div class="contact-grid"><section class="contact-information"><span class="contact-number">EMAIL</span><h3>Email addresses</h3><div class="contact-emails"><a class="email" href="mailto:jf.blazquizpulido@imtlucca.it">jf.blazquizpulido@imtlucca.it ↗</a><a class="email" href="mailto:juan.blazquiz@uv.es">juan.blazquiz@uv.es ↗</a></div></section><section class="contact-information"><span class="contact-number">ADDRESS / SPAIN</span><h3>Universitat de València</h3><address>Departament d’Anàlisi Econòmica<br>Av. dels Tarongers, s/n<br>46022 Valencia, Spain</address></section></div><section class="contact-socials"><h2>Find me online</h2>{profiles(icons=True)}</section></div>'''
